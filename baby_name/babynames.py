    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if line_num == 1:
                year = line.strip()
            if line_num != 1:
                single_line = line.split(",")   # 單行的list
                rank = single_line[0].strip()
                name1 = single_line[1].strip()
                name2 = single_line[2].strip()
...
def search_names(name_data, target):
    ...
    """
    matching_names = []
    all_name_list = []  # 所有名字的list
    target_l = target.lower()
    for name in name_data:
        all_name_list.append(name)
    for i in range(len(all_name_list)):
        if target_l in all_name_list[i].lower():
            matching_names.append(all_name_list[i])
